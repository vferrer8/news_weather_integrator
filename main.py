...
num_days_description = (
    "Number of days of the analysis period to calculate statistical weather data."
)
units_description = (
    "Units for the weather data. Possible values are: standard, metric, imperial."
)


@app.get("/articles", responses={500: {"model": ServerError}})
def collect_articles(
    topic: str = Query(..., description=topic_description),
    lat_analysis: float = Query(..., description=lat_description),
    lon_analysis: float = Query(..., description=lon_description),
    num_days_analysis: int = Query(default=5, description=num_days_description),
    weather_units=Query(default=DEFAULT_UNITS, description=units_description),
) -> list[Article]:
    try:
