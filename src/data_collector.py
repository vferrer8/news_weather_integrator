...
def get_weather_data(
    api_key: str, lat: float, lon: float, start: int, end: int, units: str
) -> dict:
...
def get_weather_data_stats(hourly_weather_data: list[dict[str, Any]]) -> pd.DataFrame:
    weather_df = pd.DataFrame(
        columns=["feels_like", "humidity", "pressure", "temp", "temp_max", "temp_min"]
    )
...
def collect_data(
    ...
    lat_analysis: float,
    lon_analysis: float,
    num_days_analysis: int = 5,
    weather_units=DEFAULT_UNITS,
):
    ...
    for news_article in news_data["articles"]:
        ...
        start_time_analysis = extract_start_timestamp(
            end_time=end_time_analysis, num_days=num_days_analysis
        )
        try:
            weather_data = get_weather_data(
                ...
                lat=lat_analysis,
                lon=lon_analysis,
                start=start_time_analysis,
                end=end_time_analysis,
                units=weather_units,
            )
        except:
            log.info("No weather data for this analysis period.")
            continue
        weather_data_stats = get_weather_data_stats(
            hourly_weather_data=weather_data["list"]
        )
        integrated_article = integrate_data(
            article=news_article, weather_stats_df=weather_data_stats
        )
